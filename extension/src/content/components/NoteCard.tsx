...
import { Play, Trash2, Copy, Check, Code, Eye, Layers, Edit3, Sparkles, ChevronDown, ChevronUp } from 'lucide-react';
...
export const NoteCard: React.FC<NoteCardProps> = ({ note, index = 0, isActivePlayback, onDelete }) => {
  ...
  const parsedBullets = (note.bulletPoints || []).map((bullet) => {
    // Check if bullet has structure like "Title: Description" or "Title - Description"
  ...
  return (
    ...
    >
          ...
          {parsedBullets.map((item, idx) => (
            <div key={idx} className="scribe-bullet-card">
              <div className="scribe-bullet-title-row">
                <span style={{ fontSize: '10px', color: '#71717a', fontFamily: 'var(--scribe-font-mono)', minWidth: '14px' }}>
                  {idx + 1}.
                </span>
                {item.label ? (
                  <span className="scribe-bullet-tag">{item.label}</span>
                ) : (
                  <span style={{ fontSize: '11.5px', color: '#d4d4d8' }}>{item.text}</span>
                )}
              </div>
      ...
      {note.diagramDescription && (
        <div style={{ fontSize: '11px', color: '#a1a1aa', fontStyle: 'italic', background: '#000000', padding: '6px 8px', borderRadius: '4px', border: '1px solid var(--scribe-border)' }}>
          📐 <strong>Visual Diagram:</strong> {note.diagramDescription}
        </div>
      )}
        ...
        <div className="scribe-code-block">
          ...
          <button
            ...
            style={{
              ...
              color: copiedCode ? '#4ade80' : '#a1a1aa',
              borderRadius: '4px',
              padding: '2px 5px',
              fontSize: '9.5px',
              cursor: 'pointer',
